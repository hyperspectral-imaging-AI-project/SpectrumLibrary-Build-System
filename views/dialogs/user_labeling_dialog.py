...
from data.db import material_add, material_code_name, send_label_add
from services.resampling_cache import update_class_in_info, load_classes_from_info, resample_cache_paths
...
class UserLabelingDialog(QtWidgets.QDialog):
    ...
    def __init__(self, parent=None, ui_dir: Optional[Path] = None):
        ...
        self._root.btnCreateNewClass.clicked.connect(self._on_create_new_class)
    ...
    def _on_create_new_class(self):
        """신규 클래스 생성 버튼 클릭 처리"""
        try:
            parent = self.parent()
            app_dir = Path(getattr(parent, "app_dir", Path(__file__).resolve().parents[3])) if parent else Path(__file__).resolve().parents[3]
            ui_dir = app_dir / "ui"
            ...
            # 시그널 연결
            dlg.material_created.connect(self._on_material_created)
            
            dlg.exec_()
            
        except Exception as e:
            import logging
            logging.exception("[UserLabeling] create new class failed")
            QtWidgets.QMessageBox.critical(self, "오류", f"다이얼로그 열기 실패:\n{e}")
    
    def _on_material_created(self, result: dict):
        """신규 클래스 생성 완료 시 클래스 옵션에 추가하고 모든 콤보박스 갱신, .info 파일에 저장"""
        try:
            mtrl_cd = result.get("mtrl_cd", -1)
            if mtrl_cd == -1:
                import logging
                logging.error("[UserLabeling] mtrl_cd가 없습니다.")
                QtWidgets.QMessageBox.critical(self, "오류", "클래스 ID(mtrl_cd)를 받지 못했습니다.")
                return
            
            # mtrl_nm(물질 이름) 가져오기 - result의 "name"이 mtrl_nm
            mtrl_nm = result.get("name", "")
            if not mtrl_nm:
                import logging
                logging.error("[UserLabeling] mtrl_nm이 없습니다.")
                QtWidgets.QMessageBox.critical(self, "오류", "물질 이름(mtrl_nm)을 받지 못했습니다.")
                return
            
            description = result.get("description", "")
            
            # MainWindow에서 primary_path와 user_type 가져오기
            # 주의: UserLabelingDialog의 parent는 MainWindow입니다 (main_window.py:1536)
            # 따라서 self.parent()가 바로 MainWindow입니다.
            parent = self.parent()
            main_window = None
            ...
            # parent가 MainWindow인지 확인
            if parent:
                try:
                    # parent가 MainWindow인 경우 (일반적인 경우)
                    if hasattr(parent, "_cache_primary_path") or hasattr(parent, "_extract_src_path"):
                        main_window = parent
                    else:
                        # parent가 다른 위젯인 경우, parent의 parent를 확인
            ...
            # ★ personal인 경우에만 .info 파일에 클래스 정보 저장
            import logging
            if user_type == "personal":
                if primary_path:
                    logging.info("[UserLabeling] .info 파일 저장 시도: primary_path=%s, mtrl_cd=%d, mtrl_nm=%s", 
                               primary_path, mtrl_cd, mtrl_nm)
                    try:
                        success = update_class_in_info(
                        ...
                        if success:
                            logging.info(f"[UserLabeling] .info 파일에 클래스 정보 저장 완료: mtrl_cd={mtrl_cd}, mtrl_nm={mtrl_nm}")
                        else:
                            logging.error(f"[UserLabeling] .info 파일 저장 실패: update_class_in_info가 False를 반환했습니다.")
                            QtWidgets.QMessageBox.warning(
                                self, 
                                "경고", 
                                f"클래스 정보를 .info 파일에 저장하지 못했습니다.\n\n"
                                f"mtrl_cd: {mtrl_cd}\n"
                                f"mtrl_nm: {mtrl_nm}\n\n"
                                f"로그를 확인하세요."
                            )
                    except Exception as e:
                        logging.exception(f"[UserLabeling] .info 파일 저장 중 예외 발생: {e}")
                        QtWidgets.QMessageBox.warning(
                            self, 
                            "경고", 
                            f"클래스 정보를 .info 파일에 저장하는 중 오류가 발생했습니다:\n{e}"
                        )
                else:
                    # personal 사용자인데 primary_path가 없을 때만 경고
                    logging.warning("[UserLabeling] personal: primary_path를 찾을 수 없어 .info 파일에 저장하지 않습니다.")
                    QtWidgets.QMessageBox.warning(
                        self, 
                        "경고", 
                        "이미지 파일 경로를 찾을 수 없어 클래스 정보를 .info 파일에 저장하지 못했습니다.\n\n"
                        "이미지를 먼저 로드한 후 다시 시도하세요."
                    )
            # server 사용자는 .info 파일에 저장하지 않으므로 경고 없음
            
            # ★ personal/server에 따라 클래스 정보를 다시 가져와서 콤보박스 업데이트
            # 단, 기존에 라벨링된 클래스(cid)는 유지하도록 보강
            class_options_updated = []
            
            # 기존 라벨링에서 사용된 모든 cid 수집 (기존 클래스 유지용)
            existing_cids_in_labels = set()
            for y, x, cid in self._pixel_labels:
                if cid >= 0:
                    existing_cids_in_labels.add(int(cid))
            print(user_type)
            if user_type == "personal":
                # personal 사용자: .info 파일에서 클래스 정보 로드
                if primary_path:
                    try:
                        class_options_updated = load_classes_from_info(primary_path)
                        if class_options_updated:
                            logging.info(f"[UserLabeling] personal: .info 파일에서 {len(class_options_updated)}개 클래스 로드 완료")
                            # 이름 캐시 업데이트
                            for cid, mtrl_nm in class_options_updated:
                                self._cid_to_name_cache[int(cid)] = mtrl_nm
                        else:
                            logging.warning("[UserLabeling] personal: .info 파일에서 클래스 정보를 읽을 수 없습니다.")
                    except Exception as e:
                        logging.exception(f"[UserLabeling] personal: .info 파일에서 클래스 정보 로드 실패: {e}")
                else:
                    logging.warning("[UserLabeling] personal: primary_path가 없어서 .info 파일에서 클래스 정보를 읽을 수 없습니다.")
            else:
                # server 사용자: API에서 클래스 정보 가져오기
                logging.info("[UserLabeling] server: API에서 클래스 정보 가져오기 시도")
                try:
                    ...
                    if api_base:
                        ...
                        for rec in api_result:
                            try:
                                ...
                                if cid >= 0 and name:
                                    class_options_updated.append((cid, name))
                                    self._cid_to_name_cache[cid] = name
                            except (TypeError, ValueError):
                                continue
                        
                        # CID 기준 정렬
                        class_options_updated.sort(key=lambda x: x[0])
                        logging.info(f"[UserLabeling] server: API에서 {len(class_options_updated)}개 클래스 로드 완료")
                    else:
                        logging.warning("[UserLabeling] server: material_code_name_url 환경변수가 설정되지 않았습니다.")
                except Exception as e:
                    logging.exception(f"[UserLabeling] server: API에서 클래스 정보 가져오기 실패: {e}")
            
            # ★ 기존 라벨링에서 사용된 cid가 새 클래스 옵션에 없으면 추가
            # NOTE: personal 경로(load_classes_from_info)에서 (cid, name, desc) 형태의 3-튜플이 들어올 수 있으므로
            # 아래에서 (cid, name) 2-튜플로 정규화한다.
            try:
                normalized: list[tuple[int, str]] = []
                for rec in class_options_updated:
                    if isinstance(rec, (list, tuple)) and len(rec) >= 2:
                        cid_i = int(rec[0])
                        name_s = str(rec[1])
                        normalized.append((cid_i, name_s))
                class_options_updated = normalized
            except Exception:
                # 정규화 실패 시, 이후 로직이 안전하게 동작하도록 빈 목록으로 초기화
                class_options_updated = []

            updated_cids = {int(cid) for cid, _ in class_options_updated}
            for existing_cid in existing_cids_in_labels:
                if existing_cid not in updated_cids:
                    # 기존 클래스 옵션에서 찾기
                    found = False
                    for cid_opt, name_opt in self._class_options:
                        if int(cid_opt) == existing_cid:
                            class_options_updated.append((existing_cid, name_opt))
                            logging.info(f"[UserLabeling] 기존 라벨링 클래스 유지: cid={existing_cid}, name={name_opt}")
                            found = True
                            break
                    
                    # 기존 클래스 옵션에도 없으면 기본 이름으로 추가
                    if not found:
                        default_name = f"Class {existing_cid}"
                        class_options_updated.append((existing_cid, default_name))
                        self._cid_to_name_cache[existing_cid] = default_name
                        logging.info(f"[UserLabeling] 기존 라벨링 클래스 추가 (기본 이름): cid={existing_cid}, name={default_name}")
            
            # CID 기준 정렬
            class_options_updated.sort(key=lambda x: x[0])
            
            # 클래스 옵션 업데이트 및 콤보박스 갱신
            if class_options_updated:
                # 클래스 옵션 업데이트
                self._class_options = class_options_updated
                ...
                if hasattr(self, "_combo_global") and self._combo_global is not None:
                    ...
                    # 콤보박스에 추가 (mtrl_nm 기준으로 표시)
                    for cid_opt, name_opt in self._class_options:
                        # 캐시에서 mtrl_nm 가져오기 (없으면 name_opt 사용)
                        display_name = self._cid_to_name_cache.get(int(cid_opt), name_opt)
                        self._combo_global.addItem(display_name, cid_opt)
                    
                    # ★ 새로 생성된 클래스를 선택하도록 설정
                    for i in range(self._combo_global.count()):
                        if int(self._combo_global.itemData(i)) == mtrl_cd:
                            self._combo_global.setCurrentIndex(i)
                            logging.info(f"[UserLabeling] 전역 콤보박스에서 새로 생성된 클래스 선택: mtrl_cd={mtrl_cd}, index={i}")
                            break
                ...
                logging.warning("[UserLabeling] 클래스 옵션을 업데이트할 수 없습니다. user_type=%s", user_type)
            
            QtWidgets.QMessageBox.information(self, "성공", f"신규 클래스 '{mtrl_nm}' (ID: {mtrl_cd})가 생성되었습니다.")
            
        except Exception as e:
            import logging
            logging.exception("[UserLabeling] material created handler failed")
            QtWidgets.QMessageBox.critical(self, "오류", f"클래스 추가 실패:\n{e}")
