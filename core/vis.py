...
def percentile_stretch(img: np.ndarray, pmin=2.0, pmax=98.0) -> np.ndarray:
    """채널별 퍼센트 스트레치(pmin–pmax%) 후 [0,1]로 스케일."""
    ...
    for c in range(x.shape[2]):
        ...
        if vmax > vmin:
            # 약간의 하이라이트 여유를 위해 vmax*1.2 사용
            b = (band - vmin) / (vmax * 1.2 - vmin)
        else:
            b = band
        y[..., c] = np.clip(b, 0.0, 1.0)
...
def make_rgb(
    ...
    if rgb_indices is not None:
        ...
        if C >= 3:
            ...
            # 2채널이면 [G,G,B] 느낌으로
            base = np.stack([x[..., 0], x[..., 0], x[..., 1]], axis=-1)
            return to_uint8(percentile_stretch(base, pmin, pmax) if stretch else base)
        else:  # C==1
            g = x[..., 0]
